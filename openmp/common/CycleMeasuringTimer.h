...
public:
  ...
  static SysClock currentTicks()
  {
    ...
    return CycleMeasuringTimer::SysClock(static_cast<float>(spec.tv_sec) * 1e9 + static_cast<float>(spec.tv_nsec));
#endif
  }
  ...
  static double secondsPerTick()
  {
    ...
    if (!fp)
    {
      fprintf(stderr, "CycleMeasuringTimer::resetScale failed: couldn't find /proc/cpuinfo.");
      exit(-1);
    }
